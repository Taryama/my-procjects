...
def calculate():    
    if request.method == "POST":
        ...
        return render_template ('result.html', var1=mynumber1, var2=mynumber2, result=lcm(int(mynumber1), int(mynumber2)), developer_name="Tarkan")
    else:
        return render_template('result.html', developer_name="Tarkan")
    

# Add a statement to run the Flask appl
if __name__== "__main__":
   
    #app.run(debug=True)
    app.run(host='0.0.0.0', port=80)
# ication which can be debugged.